from __future__ import annotations

from datetime import datetime
import pandas as pd

from core.utils import normalize_whitespace
from ingestion.crossref import PaperRecord


def build_clean_dataframe(records: list[PaperRecord], run_date: datetime) -> pd.DataFrame:
    """Clean raw records into a validated, normalized DataFrame ready for embedding and indexing."""
    rows = []
    run_date_date = run_date.date() if isinstance(run_date, datetime) else run_date

    for record in records:
        paper_id = normalize_whitespace(record.paper_id)
        title = normalize_whitespace(record.title)
        summary = normalize_whitespace(record.summary)

        # Skip invalid or empty records
        if not paper_id or not title:
            continue

        authors = [normalize_whitespace(a) for a in record.authors if normalize_whitespace(a)]
        categories = [normalize_whitespace(c) for c in record.categories if normalize_whitespace(c)]
        primary_category = record.primary_category or (categories[0] if categories else "General")

        authors_joined = ", ".join(authors)
        categories_joined = ", ".join(categories)
        summary_chars = len(summary)

        # Parse date and compute age_days
        published_str = str(record.published).strip()
        try:
            pub_date = datetime.strptime(published_str[:10], "%Y-%m-%d").date()
            age_days = max(0, (run_date_date - pub_date).days)
        except Exception:
            age_days = 0

        # Construct standard structured text for embedding
        text_for_embedding = (
            f"Title: {title}\n"
            f"Authors: {authors_joined}\n"
            f"Published: {published_str}\n"
            f"Categories: {categories_joined}\n"
            f"Summary: {summary}"
        ).strip()

        rows.append(
            {
                "paper_id": paper_id,
                "title": title,
                "summary": summary,
                "authors": authors,
                "categories": categories,
                "primary_category": primary_category,
                "published": published_str,
                "updated": str(record.updated).strip() or published_str,
                "abs_url": str(record.abs_url).strip(),
                "pdf_url": str(record.pdf_url).strip(),
                "comment": str(record.comment).strip(),
                "authors_joined": authors_joined,
                "categories_joined": categories_joined,
                "summary_chars": summary_chars,
                "age_days": age_days,
                "text_for_embedding": text_for_embedding,
            }
        )

    if not rows:
        return pd.DataFrame(
            columns=[
                "paper_id",
                "title",
                "summary",
                "authors",
                "categories",
                "primary_category",
                "published",
                "updated",
                "abs_url",
                "pdf_url",
                "comment",
                "authors_joined",
                "categories_joined",
                "summary_chars",
                "age_days",
                "text_for_embedding",
            ]
        )

    df = pd.DataFrame(rows)
    # Deduplicate based on paper_id
    df = df.drop_duplicates(subset=["paper_id"], keep="first")
    # Filter out empty records
    df = df[df["paper_id"].str.strip().ne("") & df["title"].str.strip().ne("")]
    df = df.reset_index(drop=True)
    return df

